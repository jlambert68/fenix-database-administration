...
func UpdateUsersDatabasePassword(
	...
	switch SharedCode.ExecutionEnvironment {
	case SharedCode.DEV:
		if SharedCode.UpdatePasswordInDevEnvironment == false {

			SharedCode.Logger.WithFields(logrus.Fields{
				"Id":                              "b89a7962-7d7e-45b6-86b7-d818881d8b2d",
				"SharedCode.ExecutionEnvironment": SharedCode.ExecutionEnvironment,
				"databaseUserName":                databaseUserName,
	...
	case SharedCode.ACC:
		if SharedCode.UpdatePasswordInAccEnvironment == false {
	...
	previousPasswordFromSecretManager, err = SharedCode.AccessSecretVersion(secretManagerPath + secretVersion)

	firstPasswordFromSecretManager, errFirstPassword = SharedCode.AccessSecretVersion(secretManagerPath + SharedCode.LatestSecretVersion)
	// Verify that user can log in to database with new password
	_ = ConnectToDB(databaseUserName, firstPasswordFromSecretManager, SharedCode.DBNameToTarget)
	dbPool.Close()
	...
	if err != nil {
		...
		firstPasswordFromSecretManager, errFirstPassword = SharedCode.AccessSecretVersion(secretManagerPath + SharedCode.FirstSecretVersion)
		...
		if errFirstPassword != nil {
			...
			if SharedCode.ExecutionEnvironment == SharedCode.DEV {
				SharedCode.Logger.WithFields(logrus.Fields{
					...
					"secretManagerPath+SharedCode.FirstSecretVersion": secretManagerPath + SharedCode.FirstSecretVersion,
					"previousPasswordFromSecretManager":               previousPasswordFromSecretManager,
	...
	// Verify that user can log in to database with new password
	_ = ConnectToDB(databaseUserName, previousPasswordFromSecretManager, SharedCode.DBNameToTarget)
	dbPool.Close()
	...
	secretVersionName, err = SharedCode.AddSecretVersion(secretManagerPath, newDatabasePassword)
	if err != nil {
	...
	passwordFromSecretManager, err = SharedCode.AccessSecretVersion(secretManagerPath + secretVersion)
	if err != nil {
	...
	if err != nil {
		...
		secretVersionNameForPreviousPassword, err = SharedCode.AddSecretVersion(secretManagerPath, previousPasswordFromSecretManager)
		if err != nil {
		...
		err = SharedCode.DestroySecretVersion(secretVersionName)
	...
	// Verify that user can log in to database with new password
	_ = ConnectToDB(databaseUserName, passwordFromSecretManager, SharedCode.DBNameToTarget) //passwordFromSecretManager)
	dbPool.Close()
